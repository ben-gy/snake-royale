...
function mockNet(bus: Bus, selfId: PeerId): Net {
  ...
  return {
    ...
    // bus's roster only changes when a test says so, and every such test moves
    // the clock on afterwards, so there is nothing to report here.
    onPeersChange: () => () => {},
    takeover: () => {},
    netDiag: () => ({
      selfId,
 ...
 * (01-DIAGNOSIS §3a) — the failure where a player was "ejected" the moment a
 * round began. The bus here has a settled roster, so the window is pure delay,
 * but it is real and every auto-start assertion has to wait it out.
 */
const settle = (): void => {
